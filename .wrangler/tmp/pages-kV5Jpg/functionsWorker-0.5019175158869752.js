...
async function onRequestGet({ env: env2 }) {
  try {
    const { results } = await env2.lindles.prepare(
    ...
    console.error("Message loading error:", error3);
...
async function onRequestPost({ request, env: env2 }) {
  try {
    ...
    await env2.lindles.prepare("INSERT INTO messages (name, message) VALUES (?, ?)").bind(trimmedName, trimmedMessage).run();
    return Response.json({ success: true });
  } catch (error3) {
    console.error("Message submission error:", error3);
    return Response.json(
...
async function onRequestGet2({ env: env2 }) {
  try {
    const result = await env2.lindles.prepare(
    ...
    console.error("Visitor counter error:", error3);
    return Response.json(
...
// ../.wrangler/tmp/pages-kV5Jpg/functionsRoutes-0.14182865191583338.mjs
var routes = [
  {
